...
function suma(values: any, data: any, calcParams: any) {
  let output = 0;
  const precision =
    typeof calcParams.precision !== 'undefined' ? calcParams.precision : false;

  if (values.length) {
    values.forEach((value: any) => {
      const valueNum = Number(value);

      output += !Number.isNaN(valueNum) ? valueNum : 0;
    });
  }

  return `Suma: ${String(
    precision !== false ? output.toFixed(precision) : output
  )} kg`;
}
